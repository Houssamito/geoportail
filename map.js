...
var ind_communes = new ol.layer.Image({
            title: 'communes',
...
var ind_douars = new ol.layer.Image({
            title: 'Douars',
            // extent: [-180, -90, -180, 90],
            source: new ol.source.ImageWMS({
                url: 'http://localhost:8080/geoserver/wms',
...
var ind_routes = new ol.layer.Image({
            title: 'Routes',
            // extent: [-180, -90, -180, 90],
            source: new ol.source.ImageWMS({
                url: 'http://localhost:8080/geoserver/wms',
...
overlays.getLayers().push(ind_communes);
overlays.getLayers().push(ind_douars);
overlays.getLayers().push(ind_routes);
var popup = new Popup();
map.addOverlay(popup);
...
function legend() {
    ...
    // Créer un titre principal pour la légende
    ...
    // Créer une table pour organiser les éléments de la légende en colonnes
    var legendTable = document.createElement("table");
    legendTable.style.width = '100%';

    var row = legendTable.insertRow();

    // Obtenir et trier les couches par titre, en mettant "Douars" et "Routes" en premier
    var layersArray = overlays.getLayers().getArray();
    var prioritizedLayers = layersArray.filter(layer => ['Douars', 'Routes'].includes(layer.get('title')));
    var otherLayers = layersArray.filter(layer => !['Douars', 'Routes'].includes(layer.get('title')));
    var sortedLayers = prioritizedLayers.concat(otherLayers);

    // Ajouter chaque couche à la légende
    sortedLayers.forEach(layer => {
        ...
        // Vérifier le titre de la couche et le modifier si nécessaire
        var displayTitle;
        if (layerTitle === 'Communes') {
            displayTitle = 'Communes (Douars par commune)';
        } else if (layerTitle === 'Routes') {
            displayTitle = 'Routes (par type)';
        } else {
            displayTitle = layerTitle;
        }
        
        var titleElement = document.createElement("p");
        var titleText = document.createTextNode(displayTitle);
        titleElement.appendChild(titleText);
        cell.appendChild(titleElement);

        // Créer une image pour l'icône de légende de la couche
        var img = new Image();
        img.src = "http://localhost:8080/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=PFE:" + layer.get('title');
        cell.appendChild(img);
    });

    element.appendChild(legendTable);
}
...
//var
var url = "http://localhost:8080/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=ProjetSig:Hotels&CQL_FILTER=gid<='20'&outputFormat=application/json"
...
highlightStyle = new ol.style.Style({
   ...
   featureOverlay = new ol.layer.Vector({
	...
	function  highlight2(evt) {
        ...
        if (feature) {
            ...
            var content1 = '<h3>' + feature.get('way') + '</h3>';
            content1 += '<h5>' + feature.get('amenity')+'</h5>'
            content1 += '<h5>' + feature.get('full_id') +'</h5>';
  ...
  +'<wfs:Update typeName="ProjetSig:Hotels">\n'
    +'<wfs:Property>\n'
      +'<wfs:Name>full_id</wfs:Name>\n'
      +'<wfs:Value>\n'
	  +gml3+'\n'
      +'</wfs:Value>\n'
    +'</wfs:Property>\n'
...
// Dans le HTML, ajoutez une div pour le popup
...
map.on('singleclick', function(evt) {
    ...
    overlays.getLayers().getArray().slice().forEach(function(layer) {
            ...
            if (url) {
                $.getJSON(url, function(data) {
                    if (data.features.length > 0) {
                        ...
                        info += '<h3>' + layer.get('title') + '</h3>';
                        info += '<ul>';
                        for (var key in feature.properties) {
                            info += '<li>' + key + ': ' + feature.properties[key] + '</li>';
                        }
                        info += '</ul>';
        ...
        if (layer.getVisible() && layer instanceof ol.layer.Image) {
            ...
            if (url) {
                $.getJSON(url, function(data) {
                    if (data.features.length > 0) {
                        var feature = data.features[0];
                        var title = 'Feature Info';
                        
                        // Définir le titre en fonction du type de couche
                        
                         if (layerTitle === 'communes') {
                            title = 'La commune ' + (feature.properties['com_fr'] || 'Non spécifiée');    
                        } else if (layerTitle === 'Douars') {
                            title = 'Douar numéro ' +  (feature.properties['OBJECTID'] || 'Non spécifié');
                        } else if (layerTitle === 'Routes') {
                            title = 'La route ' + (feature.properties['ROUTE'] || 'Non spécifiée');
                        }
...
function loadGeoServerStyles() {
            var styleSelect = document.getElementById('styleSelect');
            styleSelect.innerHTML = '<option value="">Loading styles...</option>'; // Show loading message

            var url = 'http://localhost:8080/geoserver/rest/styles.json';
            ...
            .then(data => {
                var styles = data.styles.style;
                styleSelect.innerHTML = ''; // Clear the loading message
        ...
        function applySymbology() {
            ...
            var styleName = document.getElementById('styleSelect').value;
            ...
            toggleSymbologyOptions();
        }

        function toggleSymbologyOptions() {
            var symbologyOptions = document.getElementById('symbologyOptions');
            if (symbologyOptions.style.display === 'none') {
                symbologyOptions.style.display = 'block';
            } else {
                symbologyOptions.style.display = 'none';
            }
        }
